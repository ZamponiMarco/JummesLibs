...
public class ProtocolWrapper_v1_19_R1 implements ProtocolWrapper {
    ...
    public void sendEntityMetadataPacket(Player p, int eid, byte tags) {
        ...
        Packet<ClientGamePacketListener> packet = new ClientboundSetEntityDataPacket(eid, watcher, true);
        ((CraftPlayer) p).getHandle().networkManager.send(packet);
    }
