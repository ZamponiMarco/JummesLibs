...
public abstract class NumberFieldChangeInventoryHolder<S extends Number> extends FieldChangeInventoryHolder {
    ...
    protected void initializeInventory() {
        ...
        registerClickConsumer(0, ItemUtils.getNamedItem(Libs.getVersionWrapper().skullFromValue(MINUS_HEAD),
                MessageUtils.color(ZOOM_IN), Lists.newArrayList()), e -> {
            scale /= 10;
            fillModifyButtons();
        });
        registerClickConsumer(8, ItemUtils.getNamedItem(Libs.getVersionWrapper().skullFromValue(PLUS_HEAD),
